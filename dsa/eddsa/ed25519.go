...
// ED25519BytesToPrivateKey unmarshals a private key as bytes into a jwk.
func ED25519BytesToPrivateKey(privateKeyBytes []byte) (jwk.JWK, error) {
	var privateKey _ed25519.PrivateKey = privateKeyBytes
	publicKey, ok := privateKey.Public().(_ed25519.PublicKey)
	if !ok {
		return jwk.JWK{}, errors.New("public key type assertion failed")
	}
